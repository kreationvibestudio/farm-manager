...
interface Vehicle {
  name: string;
  type: 'Tractor' | 'Truck' | 'Motorcycle' | 'Other';
...
interface InventoryItem {
  name: string;
  category: 'Fertilizer' | 'Herbicide' | 'Fuel' | 'Spare Part' | 'Tool' | 'Other';
  quantity: number;
  unit: string;
  min_level: number;
  last_updated: string;
...
async function populateHarvestLogs(staffIds: any[], vehicleIds: any[]) {
  ...
  // Get IDs
  const supervisor1 = staffIds.find((s: any) => s.name === 'Jane Smith')?.id;
  const supervisor2 = staffIds.find((s: any) => s.name === 'Mary Johnson')?.id;
  const supervisor3 = staffIds.find((s: any) => s.name === 'Lisa Anderson')?.id;
  const driver1 = staffIds.find((s: any) => s.name === 'Bob Wilson')?.id;
  const driver2 = staffIds.find((s: any) => s.name === 'David Brown')?.id;
  const driver3 = staffIds.find((s: any) => s.name === 'Sarah Williams')?.id;
  
  const vehicle1 = vehicleIds.find((v: any) => v.name === 'Tractor-01')?.id;
  const vehicle2 = vehicleIds.find((v: any) => v.name === 'Tractor-02')?.id;
  const vehicle3 = vehicleIds.find((v: any) => v.name === 'Truck-01')?.id;
  const vehicle4 = vehicleIds.find((v: any) => v.name === 'Truck-02')?.id;
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    harvestLogs.push({
      ...
      driver_id: driver,
      vehicle_id: vehicle,
      notes: i % 3 === 0 ? 'Good quality FFB' : i % 5 === 0 ? 'Standard harvest' : undefined,
    });
  }
